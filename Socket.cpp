...
// 设置是否允许广播，默认禁用时，向广播地址发消息是内核检测报错，发送失败
int CommonSocket::SetBroadcast(bool isbroadcast)
{
    int ret = 0;
    if ((ret = ::setsockopt(_socket_id, SOL_SOCKET, SO_BROADCAST, (const char*)&isbroadcast, sizeof(int))) == SOCKET_ERROR)
    {
        std::stringstream error;
...
int CommonSocket::SetBuffsize(int sendbuffsize, int recvbuffsize)
{
    ...
    if (sendbuffsize > 0)
    {
    	/*setsockopt:获取或者设置与某个套接字关联的选项。选项可能存在于多层协议中，它们总会出现在最上面的套接字层。当操作套接字选项时，
