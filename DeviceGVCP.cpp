...
int DeviceGVCP::DiscoveryAck()
{
    ...
    memset(pAckMsg->chManufacturerName, 0, 176);
...
//    long* pCurRegData = (long*)(cSendData + sizeof(ACK_MSG_HEADER));
     int* pCurRegAddr = (int*)(_cRecvData + sizeof(CMD_MSG_HEADER));
     int* pCurRegData = (int*)(cSendData + sizeof(ACK_MSG_HEADER));
    uint32_t nTmp;
//    for (unsigned int i = 0; i < ntohs(pCmdHdr->nLength) / sizeof(long); ++i)

    for (unsigned int i = 0; i < ntohs(pCmdHdr->nLength) / sizeof(int); ++i)
    {
//        pCurRegAddr = (long*)pCurRegAddr + i;
//        pCurRegData = (long*)pCurRegData + i;
        pCurRegAddr = (int*)pCurRegAddr + i;
        pCurRegData = (int*)pCurRegData + i;
